...
test('dev defaults to localhost:3000', () => {
...
test('accepts HTTPS targets for staging', () => {
  const proxy = getApiProxyRule('staging', {
    API_PROXY_TARGET: 'https://api.staging.example.com',
  });

  assert.equal(proxy.target, 'https://api.staging.example.com');
});
...
test('accepts HTTPS targets for prod', () => {
  ...
  assert.equal(proxy.target, 'https://api.example.com');
});
...
test('staging rejects HTTP upstreams', () => {
  assert.throws(
    () =>
      createProxy('staging', {
        API_PROXY_TARGET: 'http://api.staging.example.com',
      }),
    /HTTPS upstream/i,
  );
});
...
test('prod rejects HTTP upstreams', () => {
  assert.throws(
    () =>
      createProxy('prod', {
        API_PROXY_TARGET: 'http://api.example.com',
      }),
    /HTTPS upstream/i,
  );
});
