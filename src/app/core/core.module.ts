    ...
    return {
      ...
      providers: [
        ...
        // =======================================================================
        {
          ...
          useValue: config.websocketBaseUrl,
        },
        {
          provide: WEBSOCKET_CONFIG,
          useValue: {
            connectionTimeoutMs: config.websocket?.connectionTimeoutMs ?? 10_000,
            reconnectionAttempts: config.websocket?.reconnectionAttempts ?? Infinity,
            reconnectionDelayMs: config.websocket?.reconnectionDelayMs ?? 1_000,
            reconnectionDelayMaxMs: config.websocket?.reconnectionDelayMaxMs ?? 10_000,
            randomizationFactor: config.websocket?.randomizationFactor ?? 0.5,
          },
        },
